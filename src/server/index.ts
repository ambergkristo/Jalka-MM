...
createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'DELETE' && url.pathname === '/api/operator/third-place-qualifier-locks') {
      const permission = getManualResultPermission({
        providedSecret: singleHeaderValue(request.headers['x-results-agent-secret'])
      });
      if (!permission.allowed) return json(response, permission.status, { error: permission.error });
      const group = url.searchParams.get('group');
      if (!group) return json(response, 400, { error: 'group query parameter is required.' });
      return json(response, 200, await deleteThirdPlaceQualifierLockRuntime(group));
    }
